...
use constant COMMIT_THRESHOLD =>
  1000;                       # commit every 1000 topics on a bulk index job
...
sub new {
    ...
    $this->{url} = $Foswiki::cfg{SolrPlugin}{UpdateUrl}
...
# }
            ...
            foreach my $topic (@topics) {
                ...
                if ( $Foswiki::Plugins::SESSION->can('getApproxRevTime') ) {
                    $time =
                      $Foswiki::Plugins::SESSION->getApproxRevTime( $web,
                        $topic );
                }
                else {
...
sub indexTopic {
    ...
    if ($parent) {
        ( $parentWeb, $parentTopic ) =
          $this->normalizeWebTopicName( $web, $parent );
        $this->_addLink( \%outgoingLinks, $web, $topic, $parentWeb,
            $parentTopic );
    }
        ...
        {    # form definition found, if not the formfields aren't indexed
            ...
            if ($formFields) {
                foreach my $fieldDef ( @{$formFields} ) {
                    ...
                    if ( $seenFields{$name} ) {
                        $this->log(
"WARNING: walrofmed form definition for $web.$formName - field $name appear twice must be unique"
                        );
                        next;
                    }
                    $seenFields{$name} = 1;
                    ...
                    $this->extractOutgoingLinks( $web, $topic, $value,
                        \%outgoingLinks );
                    ...
                    if ( $type eq 'date' ) {
                        my $epoch = $value;
                        $epoch = Foswiki::Time::parseTime($value)
                          unless $epoch =~ /^\d+$/;
                        $epoch ||= 0;    # prevent formatTime to crap out
                        $value =
                          Foswiki::Time::formatTime( $epoch, 'iso', 'gmtime' );
                        $doc->add_fields( 'field_' . $name . '_dt' => $value, );
                    }

                    # multi-valued types
                    elsif ( $isMultiValued || $name =~ /TopicType/ )
                    {                    # TODO: make this configurable

                        $doc->add_fields( 'field_' 
                              . $name
                              . '_lst' => [ split( /\s*,\s*/, $value ) ] );
                    }
              ...
              # make it a text field unless its name does not indicate otherwise
                        ...
                        $doc->add_fields( $fieldName . $fieldType => $value, );
                        if ( $fieldType eq '_s' ) {
                            $doc->add_fields( $fieldName . '_search' => $value,
                            );
                        }
                    }
                }
    ...
    foreach my $sub (@Foswiki::Plugins::SolrPlugin::knownIndexTopicHandler) {
        ...
        try {
            ...
            my $e = shift;
            $this->log(
                "ERROR: while calling indexTopicHandler: " . $e->stringify() );
        };
    }
...
"{SolrPlugin}{SupportedLanguages} not defined. Please run configure."
        );
        return;
    }

    my $donePush = 0;
    if (   $web ne $this->{session}{webName}
        || $topic ne $this->{session}{topicName} )
    {
    ...
    my $prefsLanguage = Foswiki::Func::getPreferencesValue('CONTENT_LANGUAGE')
      || '';
    my $contentLanguage =
      $Foswiki::cfg{SolrPlugin}{SupportedLanguages}{$prefsLanguage} || 'detect';
...
sub extractOutgoingLinks {
    ...
    # normal wikiwords
    $text =~
s#(?:($Foswiki::regex{webNameRegex})\.)?($Foswiki::regex{wikiWordRegex}|$Foswiki::regex{abbrevRegex})#$this->_addLink($outgoingLinks, $web, $topic, $1, $2)#gexom;
...
sub commit {
    ...
    if ( $this->{commitCounter} > 1
        && ( $this->{commitCounter} >= COMMIT_THRESHOLD || $force ) )
    {
...
sub plainify {
    ...
    $text =~
s/$STARTWW((mailto\:)?[a-zA-Z0-9-_.+]+@[a-zA-Z0-9-_.]+\.[a-zA-Z0-9-_]+)$ENDWW//gm;
...
sub getContributors {
    ...
    try {
        ( undef, undef, $maxRev ) =
          $this->getRevisionInfo( $web, $topic, undef, $attachment );
    }
    catch Error::Simple with {
        my $e = shift;
        $this->log( "ERROR: " . $e->{-text} );
    ...
    # get most recent
    my ( undef, $user, $rev ) =
      $this->getRevisionInfo( $web, $topic, $maxRev, $attachment, $maxRev );
    ...
    # get creator
    ( undef, $user, $rev ) =
      $this->getRevisionInfo( $web, $topic, 0, $attachment, $maxRev );
    my $creator = getWikiName($user);
    $seen{$creator} = 1;

    for ( my $i = $maxRev ; $i >= 0 ; $i-- ) {
        my ( undef, $user, $rev ) =
          $this->getRevisionInfo( $web, $topic, $i, $attachment, $maxRev );
...
sub getWikiName {
    ...
    $wikiName = 'UnknownUser'
      unless Foswiki::Func::isValidWikiWord($wikiName)
    ;    # weed out some strangers
...
#$info->{author} = $Foswiki::Users::BaseUserMapping::DEFAULT_USER_CUID unless defined $info->{author};
        ...
        return Foswiki::Func::getRevisionInfo( $web, $topic, $rev,
            $attachment );
    }
}
        ...
        foreach my $wikiName ( keys %{ $this->{knownUsers} } ) {
            ...
            if ($topicHasPerms) {
                ...
                my $webViewPermission =
                  $this->{_webViewPermission}{$web}{$wikiName};

                unless ( defined $webViewPermission ) {
                    $webViewPermission =
                      $this->{_webViewPermission}{$web}{$wikiName} =
                      Foswiki::Func::checkAccessPermission( 'VIEW', $wikiName,
                        undef, undef, $web );
                }
